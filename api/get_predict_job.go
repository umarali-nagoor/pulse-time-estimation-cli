...
func GetPredictTime(ID string) (map[string]interface{}, error) {

	var predictedResponse map[string]interface{}

	resp, err := http.Get(utils.GetPulseUrl() + "/api/v1/predictor/" + ID)
	if err != nil || resp.StatusCode != 200 {
		return nil, fmt.Errorf("[ERROR] error executing getPredictTime.\nResponse: %+v\nError:%s", resp, err)
	}
